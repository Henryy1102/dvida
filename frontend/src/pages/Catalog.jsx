...
export default function Catalog() {
  ...
  const getCategoryColor = (categoria) => {
    const colors = {
      'agua': 'bg-cyan-100 text-cyan-800 border-cyan-300',
      'envases': 'bg-gray-100 text-gray-800 border-gray-300'
    };
  ...
  return (
    <div style={{background: '#ffffff', minHeight: '100vh'}}>
      <div className="container mx-auto px-4 sm:px-6 py-8 sm:py-12 max-w-7xl">
        <div className="bg-white border border-gray-300 rounded-card p-8 sm:p-12 mb-8 sm:mb-12 text-center" style={{borderColor: '#d1d5db'}}>
          <img src="/LOGO%20(2).jpeg" alt="d'vida" className="h-16 sm:h-20 mx-auto mb-4 object-contain rounded-lg" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {recomendados.map((p) => (
                <Link
                  key={p._id}
                  to={`/product/${p._id}`}
                  className="bg-white border border-gray-300 rounded-card p-3 hover:hover:border-gray-300 transition-all group"
                >
                  <div className="h-24 sm:h-28 flex items-center justify-center mb-3">
                    {p.imagen ? (
                      ...
                      <div className="h-20 sm:h-24 w-full bg-white rounded flex items-center justify-center text-xs text-subtext">
                        Sin imagen
                      </div>
                    )}
                  </div>
        ...
        <div className="card mb-8">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3">
            <select
              ...
              className="input px-4 py-2.5 rounded-card text-textMain text-sm transition-all"
            >
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-gray-300 rounded-card bg-white cursor-pointer hover:hover:border-gray-300 text-textMain text-sm transition-all">
              <input
                type="checkbox"
      ...
      {error && <div className="bg-red-50 text-red-700 px-4 py-2 rounded mb-4 border border-red-200">{error}</div>}
        ...
        {filteredProducts.map((p) => (
          ...
          >
            ...
            <Link to={`/product/${p._id}`} className="relative">
              <div className="p-4 bg-white rounded-t-card overflow-hidden">
                {p.imagen ? (
                  ...
                  <div className="h-48 w-full bg-white rounded flex items-center justify-center text-sm text-subtext">
                    Sin imagen
                  </div>
                )}
              </div>
              ...
              <div className="mt-auto">
                <button
                  className={`w-full py-3 px-4 rounded-card font-medium text-sm transition-all ${
                    p.stock === 0
                        ? "bg-white text-subtext cursor-not-allowed border border-gray-300"
                        : "btn-primary"
                    }`}
                  disabled={p.stock === 0}
                  onClick={(e) => {
