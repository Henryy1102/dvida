    ...
    <div style={{background: '#ffffff', minHeight: '100vh'}}>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center gap-2">
          <img src="/LOGO%20(2).jpeg" alt="d'vida" className="h-10 object-contain" />
        ...
        {/* Producto Principal */}
        <div className="bg-white rounded-lg shadow-lg p-8 mb-12 border border-gray-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex items-center justify-center bg-white rounded-lg p-8 shadow-sm">
              {product.imagen ? (
                ...
                <div className="h-96 w-full bg-white rounded-md flex items-center justify-center text-sm text-accent uppercase tracking-wide border border-gray-100">
                  Sin imagen
                </div>
              )}
            </div>
              ...
              <div className="space-y-4 mt-auto">
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      className="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-lg text-primary border border-primary/20"
                    ...
                    <input
                      ...
                      className="w-20 border border-primary rounded-lg px-4 py-2 text-center font-semibold text-lg bg-white text-textMain"
                    />
                    <button
                      className="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-lg text-primary border border-primary/20"
                      onClick={() =>
      ...
      <div className="container mx-auto px-4 pb-8">
        <div className="bg-white rounded-lg shadow-lg p-8 border border-gray-300">
          ...
          {user && (
            <div className="mb-6 border border-gray-300 rounded-lg p-4 bg-white/40">
          ...
          {reviews.length === 0 && !reviewLoading ? (
            ...
            <div className="space-y-4">
              {reviews.map((r) => (
                <div
                  key={r._id}
                  className="border border-gray-300 rounded-lg p-4 bg-white/30"
                >
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-semibold text-primary">
                      {r.usuario?.nombre || "Cliente"}
                    </p>
                    <span className="text-blue-400">{"★".repeat(r.calificacion)}</span>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {relatedProducts.map((p) => (
              <Link
                key={p._id}
                to={`/product/${p._id}`}
                className="bg-white rounded-lg shadow hover:shadow-lg hover:shadow-primary/50 transition-all duration-300 flex flex-col border border-gray-300"
              >
                {/* Imagen */}
