...
export default function AdminOrdenes() {
  ...
  const getEstadoBadgeInfo = (estado) => {
    const estadoInfo = {
      pendiente: { label: "Pendiente", color: "bg-blue-500", icon: "⏱️", desc: "Esperando confirmación" },
      ...
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
                ...
                {notificaciones.length === 0 ? (
                  ...
                  <div>
                    {notificaciones.slice(0, 10).map((notif) => (
                      <div
                        key={notif._id}
                        className={`p-4 border-b border-primary/10 hover:bg-sky-50 cursor-pointer ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-secondary rounded-lg shadow-md p-6 border border-primary/20">
            ...
            <p className="text-3xl font-bold" style={{color: '#0052a3'}} mt-2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {ordenSeleccionada ? (
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg shadow-md p-6 sticky top-4 border border-slate-200">
                <h3 className="text-xl font-bold text-slate-900 mb-4">Detalles de la Orden</h3>

                {/* Número de orden */}
                <div className="mb-4 p-3 bg-slate-50 rounded border border-slate-200">
                  <p className="text-slate-500 font-semibold text-sm">Número de Orden</p>
                ...
                <div className="mb-4 p-3 bg-white rounded border border-slate-200">
                  <p className="text-slate-600 font-semibold text-sm">Cliente</p>
                  <p className="text-slate-900 font-semibold">{ordenSeleccionada.cliente?.nombre}</p>
                  <p className="text-slate-700 text-sm">{ordenSeleccionada.cliente?.email}</p>
                </div>
                ...
                {ordenSeleccionada.ubicacion && ordenSeleccionada.ubicacion.link && (
                  <div className="mb-4 p-4 bg-white border border-slate-200 rounded">
                    <p className="text-slate-800 font-semibold mb-3">📍 Ubicación de Entrega:</p>
                    
                    {/* Link para copiar */}
                    <div className="mb-3 p-3 bg-slate-50 border border-slate-200 rounded">
                      <p className="text-slate-600 text-xs font-semibold mb-2">🔗 Link de Ubicación:</p>
                      <div className="flex gap-2 items-center">
                        <input
                          type="text"
                          value={ordenSeleccionada.ubicacion.link}
                          readOnly
                          className="flex-1 px-3 py-2 border border-slate-300 rounded text-xs bg-white text-slate-900"
                        />
                        <button
                          onClick={() => {
                    ...
                    >
                      🗺️ Ver Ubicación en OpenStreetMap
                    </a>
                  </div>
                )}
                ...
                <div className="mb-6 p-4 bg-slate-50 rounded border border-slate-200">
                  <p className="text-slate-800 font-semibold text-sm mb-4">Seguimiento del Pedido</p>
                    ...
                    {["pendiente", "procesando", "completado"].map((estado, index) => {
                      ...
                      return (
                        <div key={estado} className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <p className={`font-semibold ${isActual ? "text-slate-900" : "text-slate-700"}`}>
                              {estadoInfo.label}
                            </p>
                            <p className="text-xs text-slate-500">{estadoInfo.desc}</p>
                          </div>
                          {isActual && (
                  ...
                  <div className="mb-4">
                    ...
                    {getSiguienteEstado(ordenSeleccionada.estado) && (
                      <button
                        ...
                        className="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-primary/90 disabled:bg-gray-600 mb-3 transition flex items-center justify-center gap-2"
                      >
                ...
                <details className="mb-4">
                  ...
                  <div className="mt-3">
                    <select
                      ...
                      className="w-full px-3 py-2 bg-white border border-primary/30 rounded-lg focus:outline-none focus:border-primary text-textMain mb-3"
                    >
                    ...
                    <button
                      ...
                      className="w-full bg-primary text-white py-2 rounded-lg font-semibold hover:bg-primary/90 disabled:bg-gray-600 disabled:text-gray-400 transition"
                    >
